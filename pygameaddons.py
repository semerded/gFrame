...
class Text:
    ...
    def color(self, textColor: RGBvalue = None, backgroundColor: RGBvalue = None, borderColor: RGBvalue = None):
        if textColor != None:
...
class Drawing:
    ...
    def transparantRectangle(xPosition: float, yPosition: float, width: float, height: float, transparancy: int, color: RGBvalue = Color.LIGHT_GRAY):
        rectangle = pygame.Surface((width, height))
        rectangle.set_alpha(transparancy)
        rectangle.fill(color)
        mainDisplay.blit(rectangle, (xPosition, yPosition))
        
    def transparantRectangleFromRect(rect: pygame.Rect, transparancy: int, color: RGBvalue = Color.LIGHT_GRAY):
        rectangle = pygame.Surface((rect.width, rect.height))
        rectangle.set_alpha(transparancy)
        rectangle.fill(color)
        mainDisplay.blit(rectangle, (rect.x, rect.y))
...
class Slider:
    def __init__(self, size: list[int, int], subdivisions: int, color: RGBvalue, borderRadius: int = -1) -> None:
